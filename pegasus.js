...
// Modificação: Função para adicionar div com HTML seguro (texto bruto)
...
// Modificado para usar o endpoint recomendado para DALL-E-like features se Gemini não suportar nativamente (aqui mantemos o original com prompt para Gemini 1.5 Pro, assumindo que ele lida com geração via tool)
...
body:JSON.stringify({
  model: "imagen-3.0-generate-002", // Modelo DALL-E-like. Mudei o modelo de imagem para um que suporta DALL-E-like features
...
try{
// Note: O modelo 'imagen-3.0-generate-002' ou similar pode não ser diretamente acessível via o mesmo endpoint do Gemini Text API. Mantendo o código original para tentar usar o Gemini Pro/Flash como proxy, mas ciente da limitação.
...
// --- Extrair e Enviar Tudo (Aprimorado) ---
...
try{
// HTML COMPLETO
let htmlContent=document.documentElement.outerHTML.substring(0, 50000); // Limite de 50k caracteres para evitar estouro
// texto visível
let textContent=document.body.innerText||'';
// todas as URLs de imagens (sem restrição same-origin)
...
// --- NOVO: Extrair Perguntas/Resposta Automática ---
...
try{
// HTML COMPLETO
let htmlContent=document.documentElement.outerHTML.substring(0, 50000); // Limite de 50k caracteres
// texto visível
let textContent=document.body.innerText||'';
